...
def iterate_dataset(dataset_directory):
    ...
    for grid_folder in os.listdir(dataset_directory):
        ...
        if grid_folder != 'Grid_I':
            continue
        if not os.path.isdir(grid_folder_path):
            continue

        for data_type in ['Audio_recordings']:  # ['Audio_recordings', 'Power_recordings']
...
def create_augmented_spectro_data(file_path, n_fm, n_tm):
    ...
    for part in parts:
        # Augment
        aug = naa.NoiseAug(color='white', zone=(0, 1))
        augmented_data = aug.augment(part, 4)

        # Calculate the spectrogram
        spectro = spectrogram(part, HOP_LEN, NFFT, WINDOW)
        spectro = focus_ENF(spectro, GRID_ENF[label], MARGIN, FS, NFFT)
        spectro_data.append(spectro)

        for augmented_part in augmented_data:
            spectro = spectrogram(augmented_part, HOP_LEN, NFFT, WINDOW)
            spectro = focus_ENF(spectro, GRID_ENF[label], MARGIN, FS, NFFT)
        ...
        # Augment the data
        # Frequency masking
        '''aug = nas.FrequencyMaskingAug(factor=FACTOR)
        augmented_data_f = aug.augment(spectro, n_fm)

        # Time masking
        aug = nas.TimeMaskingAug()
        augmented_data_t = aug.augment(spectro, n_tm)'''
